import { createTask, deleteTask, searchTasksWithCommand, ensureDataDir, batchCreateOrUpdateTasks, canExecuteTask, assessTaskComplexity, updateTaskStatus, clearAllTasks } from './models/taskModel.js';
...
async function runHardTestSuite() {
    ...
    if (apiCheck.canExecute)
        throw new Error('❌ API Task should be blocked by DB Task!');
    ...
    if (!apiCheck2.canExecute)
        throw new Error('❌ API Task should be executable now!');
    ...
    if (deleteRes.success)
        throw new Error('❌ Should not simulate deletion of a dependency root!');
    ...
    if (deleteCompletedRes.success)
        throw new Error('❌ Should not delete completed task!');
    ...
    if (results.tasks.length !== 100)
        console.warn(`    ⚠️ Search returned ${results.tasks.length} results (expected 100, maybe cap? Or strict paging? Search default pg size is 5)`);
    ...
    const complexTask = await createTask('Super Complex Task', 'A very long description '.repeat(50), 'Notes '.repeat(50), [dbTask.id, apiTask.id, feTask.id] // 3 dependencies
    );
...
//# sourceMappingURL=test-hard-suite.js.map