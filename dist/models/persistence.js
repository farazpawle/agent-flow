...
class WriteBatcher {
    ...
    async flush() {
        ...
        if (this.pendingData && !this.writing) {
            this.writing = true;
            try {
                await fs.writeFile(this.filePath, JSON.stringify(this.pendingData, null, 2), "utf-8");
            }
            finally {
                this.writing = false;
 ...
 */
export function getWriteBatcher() {
    if (!writeBatcher) {
 ...
 */
export async function archiveOldTasks(tasks) {
    const now = new Date();
    ...
    if (toArchive.length > 0) {
        ...
        for (const [monthKey, monthTasks] of byMonth) {
            ...
            for (const t of monthTasks)
                taskMap.set(t.id, t);
            await fs.writeFile(archiveFile, JSON.stringify({ tasks: Array.from(taskMap.values()) }, null, 2), "utf-8");
        }
    }
    return {
 ...
 */
export async function readArchivedTasks(monthKey) {
    const archiveFile = path.join(ARCHIVE_DIR, `${monthKey}.json`);
    try {
        const data = await fs.readFile(archiveFile, "utf-8");
