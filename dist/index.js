...
import { planIdea, planIdeaSchema, analyzeIdea, analyzeIdeaSchema, reflectIdea, reflectIdeaSchema, splitTasks, splitTasksSchema, listTasksSchema, listTasks, executeTask, executeTaskSchema, verifyTask, verifyTaskSchema, completeTask, completeTaskSchema, deleteTask, deleteTaskSchema, updateTaskContent, updateTaskContentSchema, queryTask, queryTaskSchema, getTaskDetail, getTaskDetailSchema, reorderTasksTool, reorderTasksSchema, } from "./tools/taskTools.js";
// Import thought chain tools
import { processThought, processThoughtSchema, } from "./tools/thoughtChainTools.js";
// Import project tools
import { createProject, createProjectSchema, listProjects, listProjectsSchema, getProjectContext, getProjectContextSchema, deleteProject, deleteProjectSchema, } from "./tools/projectTools.js";
// Import task model functions
import { updateTaskConversationHistory, getTaskById, getAllTasks, ensureDataDir, updateTask } from "./models/taskModel.js";
import { taskEvents, TASK_EVENTS } from "./utils/events.js";
// Import client model
import { getAllClients, getClientById, registerClient, updateClientHeartbeat, markClientInactive, markAllClientsInactive, cleanupStaleClients, deleteInactiveClients } from "./models/clientModel.js";
// Import database
        ...
        if (ENABLE_GUI) {
            ...
            async function sendClientUpdate() {
                try {
                    ...
                    const count = clients.filter((c) => c.isActive).length;
                    sseClients.forEach((client) => {
                        if (!client.writableEnded) {
                            client.write(`event: client-update\ndata: ${JSON.stringify({
                                count,
                                timestamp: Date.now(),
                            })}\n\n`);
                        }
            ...
            app.delete("/api/clients/cleanup", async (req, res) => {
                try {
            ...
            app.patch("/api/clients/:id/heartbeat", async (req, res) => {
                try {
            ...
            app.post("/api/clients/notify", async (req, res) => {
                try {
            ...
            app.delete("/api/clients/:id/disconnect", async (req, res) => {
                try {
            ...
            app.patch("/api/tasks/:id", async (req, res) => {
                try {
                    ...
                    if (updates && typeof updates.status === "string") {
                        ...
                        else if (lower === "in_progress" || lower === "in progress")
            ...
            if (!portInUse && !IS_SPAWNED_GUI && !GUI_ONLY) {
                ...
                const serverProcess = spawn(process.execPath, // node executable
        ...
        server.setRequestHandler(ListToolsRequestSchema, async () => {
            return {
                tools: [
                    ...
                    {
                        name: "analyze_idea",
                        description: loadPromptFromTemplate("toolsDescription/analyzeIdea.md"),
                        inputSchema: zodToJsonSchema(analyzeIdeaSchema),
                    },
                    {
                        name: "reflect_idea",
                        description: loadPromptFromTemplate("toolsDescription/reflectIdea.md"),
                    ...
                    {
                        name: "complete_task",
                        description: loadPromptFromTemplate("toolsDescription/completeTask.md"),
                    ...
                    {
                        name: "reorder_tasks",
                        description: loadPromptFromTemplate("toolsDescription/reorderTasks.md"),
            ...
            try {
                ...
                const saveRequest = async () => {
                    if (ENABLE_DETAILED_MODE && taskId) {
                        try {
                            await updateTaskConversationHistory(taskId, 'user', JSON.stringify(request.params), request.params.name);
                        }
                        catch (error) {
                ...
                const saveResponse = async (response) => {
                    if (ENABLE_DETAILED_MODE && taskId) {
                        try {
                            await updateTaskConversationHistory(taskId, 'assistant', JSON.stringify(response), request.params.name);
                        }
                        catch (error) {
                            // Silently handle errors to avoid interrupting the main flow
                ...
                switch (request.params.name) {
                    case "plan_idea":
                        parsedArgs = await planIdeaSchema.safeParseAsync(request.params.arguments);
                        ...
                        return result;
                    case "analyze_idea":
                        parsedArgs = await analyzeIdeaSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                        ...
                        return result;
                    case "reflect_idea":
                        parsedArgs = await reflectIdeaSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                    ...
                    case "list_tasks":
                        ...
                        return result;
                    case "reorder_tasks":
                        parsedArgs = await reorderTasksSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                    ...
                    case "verify_task":
                        ...
                        return result;
                    case "complete_task":
                        parsedArgs = await completeTaskSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                        taskId = parsedArgs.data.taskId;
                    ...
                    case "process_thought":
                        ...
                        return result;
                    case "create_project":
                        parsedArgs = await createProjectSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                        ...
                        return result;
                    case "list_projects":
                        parsedArgs = await listProjectsSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                    ...
                    case "get_project_context":
                        parsedArgs = await getProjectContextSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
                        ...
                        return result;
                    case "delete_project":
                        parsedArgs = await deleteProjectSchema.safeParseAsync(request.params.arguments);
                        if (!parsedArgs.success) {
                            throw new Error(`Invalid arguments for tool ${request.params.name}: ${parsedArgs.error.message}`);
                        }
