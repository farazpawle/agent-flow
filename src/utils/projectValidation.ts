...
export async function validateProjectContext(
  ...
  if (foundProject) {
    ...
    return {
      isValid: false,
      error: `Missing 'projectId'. Found matching project via ${matchMethod}: '${foundProject.name}' (ID: ${foundProject.id}). Please retry the tool call with projectId='${foundProject.id}'.`,
    };
  }
  ...
  return {
    isValid: false,
    error: `Missing 'projectId'. No project found for current path. Please call 'list_projects' to find one or 'create_project' to register this workspace.`,
  };
}
