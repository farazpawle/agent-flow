...
export class TaskGraph {
  ...
  public topologicalSort(componentIds: string[]): string[] {
    ...
    while (queue.length > 0) {
      ...
      queue.sort((aId, bId) => {
        ...
        const orderA =
          taskA.executionOrder !== undefined ? taskA.executionOrder : Number.MAX_SAFE_INTEGER;
        const orderB =
          taskB.executionOrder !== undefined ? taskB.executionOrder : Number.MAX_SAFE_INTEGER;
  ...
  /**
   * Find the next task that is ready to start (Group 4.2).
   *
   * A task is "ready" when:
   *   - its status is PENDING (not in progress, blocked, or completed), AND
   *   - every dependency it lists is COMPLETED.
   *
   * Tasks are considered in the same ordering as `recalculateOrder` —
   * topologically sorted within each connected component, with
   * `executionOrder` then `createdAt` as tie-breakers — so the answer is
   * deterministic.
   *
   * Returns `null` when nothing is ready (everything is in flight,
   * blocked by missing deps, or already done).
   */
  public nextReady(): Task | null {
    const sorted = this.recalculateOrder();
    for (const task of sorted) {
      if (task.status !== TaskStatus.PENDING) continue;
      const depsSatisfied = task.dependencies.every((dep) => {
        const depTask = this.tasks.get(dep.taskId);
        // Missing deps are treated as unsatisfied — never claim a task
        // whose prerequisite is no longer in the graph.
        return depTask !== undefined && depTask.status === TaskStatus.COMPLETED;
      });
