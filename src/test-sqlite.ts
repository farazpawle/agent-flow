...
async function testSQLite() {
    ...
    if (!rawTask || rawTask.name !== 'Test Task') throw new Error('DB persistence failed');
    ...
    if (updated?.description !== 'Updated Desc') throw new Error('Update failed');
    ...
    if (deleted) throw new Error('Delete failed - task still exists');
    console.log('✅ Task deleted');

    // 6. Verify Events
    if (eventCount < 3) console.warn(`⚠️ Expected 3 events (Create, Update, Delete), got ${eventCount}`);
    else console.log(`✅ Events emitted: ${eventCount}`);
