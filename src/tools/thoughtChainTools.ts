...
export const processThoughtSchema = z.object({
  ...
  stage: z
    ...
    .describe(
      "Current thinking stage: problem_analysis (understand the problem), solution_design (plan approach), implementation (execute), verification (test/validate), exploration (creative discovery for UI/design), debugging (investigate errors)"
    ),
  focus: z
    ...
    .default("logic")
    .describe(
      "Focus mode: logic (strict/technical), vibe (creative/exploratory), debug (error investigation), security (auth/encryption/attack vectors), performance (optimization/profiling), accessibility (WCAG/screen readers)"
    ),
  previous_summary: z
    .string()
    .optional()
